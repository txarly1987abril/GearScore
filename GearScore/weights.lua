...
function CalculateCustomWeightScore(ItemLink, Tooltip)
	...
	for j,w in ipairs(GearScoreClassSpecList[Class]) do
		...
		Tooltip:AddLine(w.." Puntuación Personal: "..floor(SumOfStats), Red, Blue, Green)
	end
end
...
function CalculateClasicItemScore_two(ItemLink, Tooltip, Red, Green, Blue)
	...
	SumOfStats = 0
		...
		local Red, Green, Blue = GearScore_GetQuality(SumOfStats * 12.25 / GS_ItemTypes[ItemEquipLoc]["SlotMOD"])
		Tooltip:AddLine(w.." Puntuación Espec.: "..floor(SumOfStats), Red, Blue, Green)
	end
end
...
function CalculateClasicItemScore(ItemLink, Tooltip, Red, Green, Blue)
		...
		if not Class then Class, englishClass = UnitClass("player"); Class = englishClass; ShowSpecScores = false; end
		local ItemName, ItemLink, ItemRarity, ItemLevel, ItemMinLevel, ItemType, ItemSubType, ItemStackCount, ItemEquipLoc, ItemTexture = GetItemInfo(ItemLink)
		if not GearScoreClassSpecList or not GearScoreClassSpecList[Class] then
			if DEFAULT_CHAT_FRAME then DEFAULT_CHAT_FRAME:AddMessage("[GearScore] Clase no válida para puntuación: "..tostring(Class), 1, 0, 0) end
			return
		end
		if not GS_Settings["ShowSpecScores"] then return end
		if not GS_ItemTypes or not GS_ItemTypes[ItemEquipLoc] or not GS_ItemTypes[ItemEquipLoc]["SlotMOD"] then return end
		for j,w in ipairs(GearScoreClassSpecList[Class]) do
			local specKey = GearScoreClassSpecList[Class][j]
			if GS_Settings["ShowSpecScores"] and specKey and GS_Settings["ShowSpecScores"][specKey] and GearScoreSpecWeights[Class] and GearScoreSpecWeights[Class][j] and GS_ItemTypes[ItemEquipLoc] and GS_ItemTypes[ItemEquipLoc]["SlotMOD"] then
				if (GS_Settings["ShowSpecScores"][specKey] == 1) or (ShowSpecScores) then
					...
					Tooltip:AddLine(w.." Puntuación Espec.: "..floor(GearScore), Red, Blue, Green)
				end
			end
		end
end
...
function Calculate_GearScoreClasicScore(ItemLink, Class, Index)
	local ItemName, ItemLink, ItemRarity, ItemLevel, ItemMinLevel, ItemType, ItemSubType, ItemStackCount, ItemEquipLoc, ItemTexture = GetItemInfo(ItemLink)
	local GS_TempBonuses = BonusScanner:ScanItem(ItemLink)
	if not GearScoreSpecWeights or not GearScoreSpecWeights[Class] or not GearScoreSpecWeights[Class][Index] then return 0 end
	if not GS_TempBonuses then return 0 end
	if ( ItemEquipLoc == "INVTYPE_TRINKET" ) or ( ItemEquipLoc == "INVTYPE_RELIC" ) or ( ItemEquipLoc == "INVTYPE_SHIELD" ) or ( ItemEquipLoc == "INVTYPE_HOLDABLE" ) then
		...
		for i,v in pairs(GS_TempBonuses) do
			v = v * ( GearScoreClassStats and GearScoreClassStats[i] or 0 )
			local specWeight = GearScoreSpecWeights[Class][Index][i] or 0
			SumOfNegativeStats = SumOfNegativeStats + ((v - (v * specWeight))^ 1.7095)
		end
...
function GearScoreClassScan(Name)
	...
	if ( GS_Data[GetRealmName()].Players[Name] ) or LookAtCurrentTarget then
		...
		for i,v in pairs(PlayersSumBonuses) do
			...
			if PlayersSumBonuses and not ( string.find(i, "DPS") ) then 
				local translatedStat = GearScoreClassStatsTranslation[i] or i
				tooltip:AddLine("+"..PlayersSumBonuses[i], translatedStat)
			end
...
function GS_UpdateStatTip(PlayersSumBonuses)
	if not PlayersSumBonuses then return end
	
	-- Crear una tabla local para mostrar las estadísticas traducidas
	local translatedStats = {}
	
	-- Recorrer todas las estadísticas y traducirlas
	for statKey, statValue in pairs(PlayersSumBonuses) do
		local translatedKey = GearScoreClassStatsTranslation[statKey] or statKey
		translatedStats[translatedKey] = statValue
	end
	
	-- Si hay una interfaz que muestre estas estadísticas, actualizarla
	-- Esta función puede ser llamada desde el código principal para mostrar stats traducidas
...
function GS_CalculateSpecScore(Name, Class)
	...
	_G["GS_SpecBar"..j.."GearSpecText"]:SetText(GearScore_GetTranslatedSpecName(GearScoreClassSpecList[Class][j]))
	...
	if ( Class ~= "DRUID" ) and ( GearScoreSpecWeights[Class][i]["DEFENSECAP"] ) then
		if ( SpecBonuses["DEFENSE"] ) then
			if ( (SpecBonuses["DEFENSE"] / 4.92) < 140  ) then 
				...
				table.insert(GS_WeightsTips[i], { [1] = "BAJO CAP DE DEFENSA", [2] = -1 * PercentReduction } )
			end
		else
			SumStats = SumStats * 0.75
			table.insert(GS_WeightsTips[i], { [1] = "BAJO CAP DE DEFENSA", [2] = -0.25 } )
		end
	end
	if ( GearScoreSpecWeights[Class][i]["ABSOLUTEMINHIT"] ) then
		if ( SpecBonuses["TOHIT"] ) then
			if ( SpecBonuses["TOHIT"]  < GearScoreSpecWeights[Class][i]["ABSOLUTEMINHIT"] ) then 
				...
				table.insert(GS_WeightsTips[i], { [1] = "BAJO CAP DE GOLPE", [2] = -1 * PercentReduction } )
			end
		else
...
if ( Class == "DEATHKNIGHT" ) or ( Class == "WARRIOR" ) or ( Class == "Rogue" ) or ( Class == "Shaman" ) then
 --Force Tanks to not use Duel Weilding,
	...
	if ItemLink and ItemEquipLoc ~= "INVTYPE_SHIELD" then
		local itemScore = GearScore_GetItemScore(ItemLink) or 0
		SumStats = SumStats - itemScore
		table.insert(GS_WeightsTips[i], { [1] = "DOBLE EMPUÑADURA", [2] = ((-1 * itemScore)/TotalStats) } )
	end
	end
	...
	if ItemLink then
		local itemScore = GearScore_GetItemScore(ItemLink) or 0
		SumStats = SumStats - ( 0.5 * itemScore)
		table.insert(GS_WeightsTips[i], { [1] = "SIN DOBLE EMPUÑADURA", [2] = ((-1 * ( 0.5 * itemScore))/TotalStats) } )
	end
...
if ( Class == "WARRIOR" ) and ( SuperTotallyFakeVariable2 ) then
 --Forces Fury Warriors to Dual Weild and Arms / Tanks not to.
	...
	if ( GearScoreSpecWeights[Class][i]["DUEL"] == 1 ) and ( GS_Data[GetRealmName()]["Players"][Name]["Equip"][17] == "0:0" ) then
		...
		table.insert(GS_WeightsTips[i], { [1] = "SIN DOBLE EMPUÑADURA", [2] = ((-1 * ( 0.5 * GearScore_GetItemScore(ItemLink)))/TotalStats) } )
		--print("RemovingStats for not wearing DuelWeild")
	end
end
	...
	_G["GS_SpecBar"..SpecSlot.."GearSpecText"]:SetText(GearScore_GetTranslatedSpecName(GearScoreClassSpecList[Class][SpecSlot]))

	if ( GearScoreSpecWeights[Class][i]["HITCAP"] ) then
			GS_SpecialWeightsTips[i] =  GearScoreSpecWeights[Class][i]["HITCAP"] 
			local indexname = "|CFFF0F0FF"..Name.." Índice de Golpe (Del Equipo):"
	...
	if ( GearScoreSpecWeights[Class][i]["DEFENSECAP"] ) then
			...
			if ( GS_SpecialWeightsTips[i][3]["Aguante (Estadística)"] ) then GS_SpecialWeightsTips[i][3]["Aguante (Estadística)"] = "+"..floor( ( (SpecBonuses["RESILIENCE"] or 0) / 82 ) * 25 ); end
			local indexname = "|CFFF0F0FF"..Name.." Habilidad de Defensa (Del Equipo):"
...
GearScoreSpecWeights = {
	["PALADIN"] = {
		...
		[2] = {--Protección
			["DEFENSE"] = 1,
			["DEFENSECAP"] = { [1] = { ["Cap de defensa total"] = "540" }, [3] = { ["Aguante (Estadística)"] = "XXX" } },
			["STR"] = 1,
			["AGI"] = 1,
			["BLOCK"] = 1,
			["DODGE"] = 1,
		...
		[3] = {--Reprensión
			...
			["SPELLPOW"] = 1,
			["TOHITMIN"] = 263,
			["HITCAP"] = {[1] = { ["CAP indice de golpe"] = "8.00%" }, [3] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
	...
	["DRUID"] = {
		[1] = {--Equilibrio
			...
			["INT"] = 1,
			["ATTACKPOWER"] = 0,
			["HITCAP"] = {[1] = { ["CAP de indice de golpe"] = "17.00%" }, [3] = { ["Equilibrio de poder (Talento)"] = "+4.00%" }, [4] = { ["Fuego feérico mejorado (Talento + Hechizo)"] = "+3.00%" }, [5] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
		...
		[2] = {--Feral (DPS)
			...
			["HASTE"] = 1,
			["SPELLPOW"] = 0,
			["HITCAP"] = {[1] = { ["Índice de golpe mínimo"] = "8.00%" }, [3] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 229.53,
			["CAPRATIO"] = 32.79,
			["OVERHITCAP"] = 0,
			["STA"] = 1,
		...
		[3] = {--Feral (Tanque)
			...
			["ATTACKPOWER"] = 1,
			["DODGE"] = 1,
			["DEFENSECAP"] = { [1] = { ["CAP de índice de defensa"] = "540" }, [3] = { ["Aguante (Estadística)"] = "XXX" }, [4] = { ["Supervivencia del más apto (Talento)"] = "+150" } },
			["DEFENSE"] = 1,
			["ARMORPEN"] = 1,
			["HASTE"] = 1,
			["SPELLPOW"] = 0,
	...
	["DEATHKNIGHT"] = {
		[1] = {--Sangre
			...
			["EXPERTISE"] = 1,
			["STR"] = 1,
			["HITCAP"] = {[1] = { ["CAP de golpe"] = "8.00%" }, [3] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 229.53,
			["CAPRATIO"] = 32.79,
			["DUEL"] = 0,
			["AGI"] = 1,
		...
		[3] = {--Escarcha (DPS)
			...
			["EXPERTISE"] = 1,
			["STR"] = 1,
			["HITCAP"] = {[1] = { ["CAP de golpe"] = "8.00%" }, [3] = { ["Nervios de acero frío (Talento)"] = "+3.00%" },  [4] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
	...
	["WARRIOR"] = {
		[1] = {--Armas
			...
			["STA"] = 1,
			["DUEL"] = 0,
			["HITCAP"] = {[1] = { ["CAP de índice golpe"] = "8.00%" }, [3] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 229.53,
			["CAPRATIO"] = 32.79,
		},
		[2] = {--Furia
			...
			["SPELLPOW"] = 0,
			["BLOCK"] = 0,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe"] = "8.00%" }, [3] = { ["Precisión (Talento)"] = "+3%" }, [4] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 131.16,
			["CAPRATIO"] = 32.79,
		},
		[3] = {--Protección
	...
	["PRIEST"] = {
		...
		[3] = {--Sombras
			...
			["MANAREG"] = 1,
			["ATTACKPOWER"] = 0,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe"] = "17.00%" }, [3] = { ["Concentración en las Sombras (Talento)"] = "+3.00%" }, [4] = { ["Desgracia (Talento)"] = "+3.00%" }, [5] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
	...
	["WARLOCK"] = {
		[1] = {--Aflicción
			...
			["MANAREG"] = 0,
			["ATTACKPOWER"] = 0,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe"] = "17.00%" }, [3] = { ["Supresión (Talento)"] = "+3.00%" }, [4] = { ["Fuego feérico / Desgracia (Perjuicio)"] = "+3.00%" }, [5] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 262.3,
			["CAPRATIO"] = 26.23,			
			["OVERHITCAP"] = 1,
		},
	...
	["ROGUE"] = {
		[1] = {--Combate
			...
			["EXPERTISE"] = 1,
			["AGI"] = 1,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe"] = "8.00%" }, [3] = { ["Precisión (Talento)"] = "+5.00%" }, [4] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
		...
		[3] = {--Sutileza
			...
			["EXPERTISE"] = 1,
			["AGI"] = 1,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe mínimo"] = "8.00%" }, [3] = { ["Precisión (Talento)"] = "+5.00%" }, [4] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 65.58,
			["CAPRATIO"] = 32.79,
		},
	},
	["HUNTER"] = {
		[1] = {--Puntería
			...
			["RANGEDCRIT"] = 1,
			["INT"] = 1,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe mínimo"] = "8.00%" }, [3] = { ["Puntería concentrada (Talento)"] = "+3.00%" }, [4] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 131.16,
			["CAPRATIO"] = 32.79,
		},
	...
	["MAGE"] = {
		[1] = {--Arcano
			...
			--["MANAREG"] = 0,
			["SPI"] = 1,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe"] = "17.00%" }, [3] = { ["Concentración Arcana (Talento)"] = "+3.00%" }, [4] = { ["Precisión (Talento)"] = "+3.00%" },  [5] = { ["Fuego feérico / Desgracia (Perjuicio)"] = "+3.00%" }, [6] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
		...
		[2] = {--Fuego
			...
			--["MANAREG"] = 0,
			["SPI"] = 1,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe"] = "17.00%" }, [3] = { ["Fuego feérico / Desgracia (Perjuicio)"] = "+3.00%" }, [4] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
		...
		[3] = {--Escarcha
			...
			--["MANAREG"] = 0,
			["SPI"] = 1,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe requerido"] = "17.00%" }, [3] = { ["Precisión (Talento)"] = "+3.00%" }, [4] = { ["Fuego feérico / Desgracia (Perjuicio)"] = "+3.00%" }, [5] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 262.3,
			["CAPRATIO"] = 26.23,
		},
	},
	...
	["SHAMAN"] = {
		[1] = {--Elemental
			...
			["INT"] = 1,
			["BLOCK"] = 1,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe requerido"] = "17.00%" }, [3] = { ["Precisión elemental (Talento)"] = "+3.00%" }, [4] = { ["Fuego feérico / Desgracia (Perjuicio)"] = "+3.00%" }, [5] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
			["ABSOLUTEMINHIT"] = 262.3,
			["CAPRATIO"] = 26.23,
			["BLOCKVALUE"] = 1,
			["OVERHITCAP"] = 1,
		...
		[2] = {--Enhancement
			...
			["ARMORPEN"] = 1,
			["BLOCK"] = 0,
			["HITCAP"] = {[1] = { ["CAP de índice de golpe mínimo"] = "8.00%" }, [3] = { ["Especialización en doble empuñadura (Talento)"] = "+6.00%" }, [4] = { ["Presencia heroica de draenei (Racial)"] = "+1.00%" } },
...
GearScoreClassStatsTranslation = {
	...
	["BLOCK"] = "Índice de bloqueo",["BLOCKVALUE"] = "Bloqueo de escudo",["DODGE"] = "Índice de esquiva",["PARRY"] = "Índice de parada",["RESILIENCE"] = "Aguante",
	["ARMORPEN"] = "Penetración de armadura",["EXPERTISE"] = "Pericia",["DEFENSE"] = "Índice de defensa",["ATTACKPOWER"] = "Poder de ataque",
	["RANGEDATTACKPOWER"] = "Poder de ataque a distancia",["CRIT"] = "Índice de golpe crítico",["RANGEDCRIT"] = "Índice de crítico a distancia",
	["TOHIT"] = "Índice de golpe",["RANGEDHIT"] = "Índice de golpe a distancia",["HASTE"] = "Celeridad",["SPELLPEN"] = "Poder con hechizos",["SPELLPOW"] = "Poder con hechizos",
	["MANAREG"] = "Maná cada 5",
}

-- Tabla de traducción para especializations (solo para mostrar en UI)
GearScoreSpecTranslations = {
	-- Shaman
	["Elemental"] = "Elemental",
	["Enhancement"] = "Mejora",
	...
	-- Death Knight
	["Blood (DPS)"] = "Sangre (DPS)",
	["Unholy (DPS)"] = "Profano (DPS)",
	["Frost (DPS)"] = "Escarcha (DPS)",
	["Tanking"] = "Tanque",
...
-- Función helper para obtener nombre traducido de especialización
function GearScore_GetTranslatedSpecName(specName)
	return GearScoreSpecTranslations[specName] or specName
end
