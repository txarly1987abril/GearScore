...
local baseID, enchantID, gem1ID, gem2ID, gem3ID, gem4ID, suffixID, instanceID = itemLink:match("item:(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+):(%-?%d+)")
	...
	if BonusScannerConfig.basiciteminfo == 1 then
		-- Removed Item ID display, only showing item level
		tooltip:AddLine(_G["LIGHTYELLOW_FONT_COLOR_CODE"]..L["BONUSSCANNER_ILVL_LABEL"]..itemLevel)
		tooltip:Show()
	end
	
	if BonusScannerConfig.extendediteminfo == 1 then
		-- Removed enchant and gem IDs display to hide all ID information
